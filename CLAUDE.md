...
## Правило обновления дашборда (ОБЯЗАТЕЛЬНО)

Когда мы с Павлом завершаем любую тему, задачу или функцию — СРАЗУ:
1. Получить текущее время: `TZ=Asia/Jerusalem date +"%d.%m.%Y %H:%M"`
2. Добавить/обновить пункт в `done[]` нужного агента в `assistant/web.py`
3. Закоммитить и запушить

Делать БЕЗ напоминания со стороны Павла — это часть завершения любой задачи.
Формат timestamp: `"04.05.2026 18:45"` — всегда Israel time (UTC+3 летом).
...
- **Каналы:** дистрибуторы + прямой B2B (старт Одесса)
...
### Сайт
- **Статус:** не создан. Цель: B2B витрина (не интернет-магазин).
- **Платформа:** Tilda → Shopify когда нужна оплата.
- **Структура:** Главная / Продукты / B2B / О бренде / Контакты

### Агенты HairLove
| Команда | Роль |
|---|---|
| `/hairlove` | Оркестратор — маршрутизация |
| `/hairlove-texts` | Копирайтер — тексты для всех каналов |
| `/hairlove-insta` | Instagram — публикация постов |
| `/hairlove-strategy` | Стратег — дорожная карта |
| `/hairlove-site` | Сайт — создание и тексты |
| `/hairlove-competitors` | Конкуренты — анализ рынка |
| `/hairlove-ads` | Реклама — Meta Ads |
